    ...
    while True:
        ...
        for i in range(len(X)):
            ...
            for j, index in enumerate(indices):
                x_patch = x[index[0]:index[0]+patch_shape[0], index[1]:index[1]+patch_shape[1], index[2]:index[2]+patch_shape[2]]
                y_patch = y[index[0]:index[0]+patch_shape[0], index[1]:index[1]+patch_shape[1], index[2]:index[2]+patch_shape[2]]
...
def get_data(patients_list, standard_spacing, HU_window):
    X = list()
    Y = list()
...
def train(model_dir, data_path, patch_shape, overlap, HU_window, n_classes=6, train_batch_size=8, valid_batch_size=8,
          learning_rate=0.001, standard_spacing=None, num_folds=None, id_fold_test=None):
    ...
    X_train, Y_train = get_data(patients_train, standard_spacing, HU_window)
    X_valid, Y_valid = get_data(patients_valid, standard_spacing, HU_window)
    print("train patients:", len(X_train))
    print("valid patients:", len(X_valid))
    gen_train = data_generate(X_train, Y_train, patch_shape, overlap=overlap, batch_size=train_batch_size, n_classes=n_classes, random=True)
    gen_valid = data_generate(X_valid, Y_valid, patch_shape, overlap=overlap, batch_size=valid_batch_size, n_classes=n_classes)
    steps_train = get_steps(X_train, patch_shape, overlap=overlap, batch_size=train_batch_size, random=True)
    steps_valid = get_steps(X_valid, patch_shape, overlap=overlap, batch_size=valid_batch_size)  
    ...
    callbacks = get_callbacks(model_file, initial_learning_rate=learning_rate, learning_rate_drop=0.5, learning_rate_epochs=None,
                  learning_rate_patience=30, logging_file=logging_file, verbosity=1, early_stopping_patience=50)
...
config = dict()
config["data_path"] = os.path.join("..", "data", "LCTSC")
config["result_path"] = os.path.join("..", "result", "LCTSC")
config["model_name"] = "Unet3D_LCTSC_Patch-96-96-96_HU--200-300_Resolution_2_2_2.5_fold"
config["standard_spacing"] = (2, 2, 2.5)
config["HU_window"] = (-200, 300)
config["patch_shape"] = (96, 96, 96)
config["overlap"] = (48, 48, 48)
config["n_classes"] = 6
...
if __name__ == "__main__":
    for i in range(5):
        ...
        train(model_dir=config["model_dir"], 
            ...
            valid_batch_size=4,
            learning_rate=5e-4,
            standard_spacing=config["standard_spacing"],
            num_folds=5,
            id_fold_test=i)